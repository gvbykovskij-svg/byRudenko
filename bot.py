...
def add_rating_history(user_id, user_name, change_amount, new_rating, comment=None):
    """Добавление записи в историю изменений рейтинга"""
    conn = get_db_connection()
    cursor = conn.cursor()

    cursor.execute('''
                   INSERT INTO rating_history (user_id, user_name, change_amount, new_rating, comment)
                   VALUES (?, ?, ?, ?, ?)
                   ''', (user_id, user_name, change_amount, new_rating, comment))
...
# Хранилище для ожидаемых комментариев
waiting_for_comment = {}
...
def history_command(message):
    ...
    cursor.execute('''
                   SELECT user_name, change_amount, new_rating, comment, created_at
                   FROM rating_history
    ...
    if not history_records:
        ...
        for record in reversed(history_records):
            user_name, change_amount, new_rating, comment, created_at = record
            change_emoji = "📈" if change_amount > 0 else "📉"
            change_symbol = "+" if change_amount > 0 else ""
            comment_text = f" 💬 {comment}" if comment else ""
            history_text += f"{change_emoji} {user_name}: {change_symbol}{change_amount}{comment_text}\n"
...
def callback_handler(call):
    ...
    if call.data == "start_in_chat":
        ...
        if user_id == target_user_id:
            ...
            # Сохраняем информацию о действии и запрашиваем комментарий
            ...
            bot.send_message(
                chat_id,
                f"💬 {user_name}, напишите комментарий почему вы повышаете рейтинг:\n\n"
                "📝 Примеры:\n"
                "• 'Хорошо поработал'\n"
                "• 'Помог коллегам'\n"
                "• 'Отличная идея'\n"
        ...
        bot.send_message(
            chat_id,
            f"💬 {user_name}, напишите комментарий почему вы понижаете рейтинг:\n\n"
            "📝 Примеры:\n"
            "• 'Опоздал на встречу'\n"
            "• 'Не выполнил задачу'\n"
...
def handle_text_messages(message):
    """Обработчик текстовых сообщений для комментариев и обычных запросов"""
    user_id = message.from_user.id
    chat_id = message.chat.id

    # Проверяем, ожидаем ли мы комментарий от этого пользователя
    if user_id in waiting_for_comment:
        action_data = waiting_for_comment[user_id]
        ...
        if len(comment) > 200:
            bot.send_message(
                chat_id,
                "❌ Комментарий слишком длинный (максимум 200 символов). Попробуйте снова:"
            )
            return

        if action_data['action'] == 'increase':
            current_rating = get_bot_data('rating', 0)
            ...
            add_rating_history(user_id, action_data['user_name'], 1, new_rating, comment)
            bot.send_message(
                chat_id,
                f"📈 {action_data['user_name']} повысил рейтинг слоняры!\n"
                f"💬 Комментарий: {comment}\n"
                f"🎯 Новый рейтинг: {new_rating}"
            )
            logger.info(f"Рейтинг повышен пользователем {action_data['user_name']} с комментарием: {comment}")

        elif action_data['action'] == 'decrease':
            current_rating = get_bot_data('rating', 0)
            new_rating = current_rating - 1
            set_bot_data('rating', new_rating)
            ensure_rating_limits()
            ...
            bot.send_message(
                chat_id,
                f"📉 {action_data['user_name']} {action} рейтинг слоняры!\n"
                f"💬 Комментарий: {comment}\n"
                f"🎯 Новый рейтинг: {new_rating}"
            )
            logger.info(f"Рейтинг понижен пользователем {action_data['user_name']} с комментарием: {comment}")

        # Удаляем пользователя из ожидания комментария
        del waiting_for_comment[user_id]

    else:
        # Обычная обработка текстовых сообщений (как было)
        if message.chat.type in ['group', 'supergroup']:
